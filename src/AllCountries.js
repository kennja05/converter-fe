        ...
        return(
            <div className='all-countries container'>
                ...
                <ul className='list-group countries-list'>
                    {this.state.countries.slice(this.state.startIndex, this.state.startIndex + 50).map(country => <li className='list-group-item' key={country.id}><span>{country.name}</span> - <span>{country.currency_name} <span className='country-link' onClick={this.handleCodeClick} id={country.currency_code}>({country.currency_code})</span></span><Link to={{pathname: '/convert', state:{convertFrom: country}}}>oijoij</Link></li>)}
                </ul>
